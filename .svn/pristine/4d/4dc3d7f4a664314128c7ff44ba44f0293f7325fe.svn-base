...
(function ($) {
    var defaultSettings = {
        ...
        submitFilesNum: 5,//最大提交上传数（当触发submitUpload方法时，文件上传的个数）
    ...
    $.fn.uploadFile = function (settings) {
        ...
        return this.each(function () {
            ...
            self.bind("click", function (e) {
                upload.chooseFile(settings.perviewElementId);
            });
...
UploadAssist.prototype = {
    ...
    createInputFile: function () {
        ...
        input.onchange = function () {
            ...
            //检查是否为允许上传的文件
            if (that.settings.isValitefileSuffixs) {
            ...
            if (that.settings.isGetFileSize) {
                if (this.files[0].size > that.settings.maxFileSize)
                {
                    var size = this.files[0].size / 1024;
                    alert("大小超过最大限制"+size+"M")
                    return;
                }
            }
            var fileSize;
            ...
            if ($("#" + that.settings.perviewElementId).find("li").length < that.settings.maximumFilesUpload) {
                $("#" + that.settings.perviewElementId).find("li[init]").remove();
            }
            else {
                $("#" + that.settings.perviewElementId).find("li").eq(0).remove();
            }
    ...
    chooseFile: function () {
        var uploadfile;
        if ($("#" + this.settings.perviewElementId).find("li").length < this.settings.maximumFilesUpload) {
            //$("#" + this.settings.perviewElementId).find("li[init]").remove();
            uploadfile = this.getUploadInput();
            $(uploadfile).click();
        }
        else {
            if (this.choseFilePath.length < this.settings.maximumFilesUpload) {
                //$("#" + this.settings.perviewElementId).find("li").eq(0).remove();
                uploadfile = this.getUploadInput();
                $(uploadfile).click();
            }
            else {
...
var perviewImage = {
    ...
    imageOperation: function (/*文件上传控件实例*/file, /*需要显示的元素id或元素实例*/perviewElemId,/*上传页面所在的document对象*/ dcmt) {
        ...
        if (file.files && file.files[0]) { //此处为Firefox，Chrome以及IE10的操作
            ...
            img.onload = function () {
                var rect = perviewImage.clacImgZoomParam(MAXWIDTH, MAXHEIGHT, img.offsetWidth, img.offsetHeight);
                //var rect = perviewImage.clacImgZoomParam(MAXWIDTH, MAXHEIGHT, MAXWIDTH, MAXHEIGHT);//非等比缩放
                img.style.width = rect.width + 'px';
                img.style.height = rect.height + 'px';
                img.style.marginLeft = rect.left + 'px';
                img.style.marginTop = rect.top + 'px';
    ...
    createPreviewElement: function (/*关闭图片名称*/name,/*上传时的文件名*/file, /*预览时的样式*/style) {
        ...
        var top = 0 - window.parseInt(style.height) - 15;
